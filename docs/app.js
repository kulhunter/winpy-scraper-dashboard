...
async function fetchProducts() {
  try {
    ...
    if (!res.ok) throw new Error('Error fetching products from API');
    products = await res.json();
    isStaticMode = false;
    finishProductLoading();
  } catch (err) {
    console.warn("Express API connection failed. Trying static products.json fallback...", err);
    try {
      const res = await fetch('./products.json');
      if (!res.ok) throw new Error('Error fetching static products.json');
      products = await res.json();
      isStaticMode = true;
      showSystemLog('Cargado en Modo Estático (desde products.json). Sincronización en vivo desactivada.');
      enableStaticModeUI();
      finishProductLoading();
    } catch (staticErr) {
      console.error(staticErr);
      showSystemLog('Error crítico: No se pudo cargar el catálogo de productos local ni el estático.', true);
    }
  }
}
...
function enableStaticModeUI() {
  ...
  indicator.className = 'header-sync-status idle';
  text.textContent = 'Modo Estático (Lectura)';
  
  // Update control form help text in Scraper tab
  const progressMsg = document.getElementById('progress-status-msg');
  if (progressMsg) {
    progressMsg.textContent = 'El raspador en vivo requiere un servidor local. Para actualizar los datos, ejecute el raspador localmente y haga git push.';
  }
